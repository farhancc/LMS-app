...
export const createLayout = CatchAsyncError(async (req, res, next) => {
    ...
    if (type === "FAQ") {
        const { faq } = req.body;
        const faqItems = await Promise.all(faq.map((el) => {
    ...
    if (type === "Categories") {
        ...
        const categoryItems = await Promise.all(categories.map((el) => {
...
export const editLayout = CatchAsyncError(async (req, res, next) => {
    ...
    if (type === "banner") {
        const bannerdoc = await Layout.find({ type: "banner" });
    ...
    if (type === "FAQ") {
        const Faqdoc = await Layout.find({ type: "FAQ" });
        const { faq } = req.body;
        const faqItems = await Promise.all(faq.map((el) => {
            return { question: el.question, answer: el.answer };
        }));
    ...
    if (type === "Categories") {
        const Faqdoc = await Layout.find({ type: "Categories" });
        const { categories } = req.body;
        const categoryItems = await Promise.all(categories.map((el) => {
            return { title: el.title };
        }));
